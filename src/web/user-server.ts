...
import { IConfig, IScoreSubmission, IUserInfo } from '../types'
...
export const runUserServer = (config: IConfig): Promise<void> => {
  const app = express()
  ...
  app.post('/newuser', (req, res) => {
    ...
    if (username.length > 20) {
      returnError(
        res,
        `Username must be 20 characters or less (yours is ${username.length})`
      )
      return
    }
  ...
  app.post('/score', (req, res) => {
    ...
    if (
      !checkInvalidKeys(
        score,
        ['uniqueUserId', 'beatmapKey', 'score', 'accuracy', 'fc'],
        (missedKeys) => {
          returnError(
            res,
            `Missing/Invalid key values for score: ${missedKeys.join(',')}`
          )
          return
        }
      )
    ...
    registerScoreUserId(score.beatmapKey, score.uniqueUserId, {
      score: score.score,
